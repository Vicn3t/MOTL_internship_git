...
TCGATargetDataPrefiltering <- function(view, brcds_SS, SS, YTrgFull, Fctrzn) {
  ...
  FtrsLrn = Fctrzn@features_metadata$feature[
    Fctrzn@features_metadata$view == view
  ]
...
TCGATargetDataPreparation <- function(
  views,
  YTrgFull,
  brcds_SS,
  SS,
  ...
  YTrgSSFull = sapply(
    views,
    function(view, brcds_SS, SS, YTrgFull, Fctrzn) {
  ...
  YTrgSSFull = sapply(
    views,
    function(view, YTrgSSFull, smpls) {
...
TargetDataPrefiltering <- function(view, YTrg_list, Fctrzn, smpls) {
  ...
  print(paste0("YTrg ", view, " dimensions before prefiltering: ", dim(YTrg)))

  ## prefiltering, only condition is that variance >0
  FtrsKeep = rowVars(YTrg, na.rm = TRUE) > 0
  FtrsKeep[is.na(FtrsKeep)] = FALSE
  YTrg = YTrg[FtrsKeep, ]
  print(paste0("YTrg ", view, " dimensions after prefiltering: ", dim(YTrg)))
  ...
  print(paste0(
    "YTrg ",
    view,
    " features length after TCGA matching: ",
    length(rownames(YTrg))
  ))
...
TargetDataPreparation <- function(
  views,
  YTrg_list,
  Fctrzn,
  smpls,
  ...
  ## Feature variance prefiltering and feature harmonization
  print("Feature prefiltering")
  YTrg_list = sapply(views, TargetDataPrefiltering, YTrg_list, Fctrzn, smpls)

  zero_samples <- unique(unlist(
    lapply(YTrg_list, function(mat) {
      bad <- which(colSums(mat > 0, na.rm = TRUE) == 0)
      if (length(bad) > 0) colnames(mat)[bad] else NULL
    })
  ))

  if (length(zero_samples) > 0) {
    message("Dropping all-zero samples: ", paste(zero_samples, collapse = ", "))
    YTrg_list <- lapply(YTrg_list, function(mat) {
      mat[, !colnames(mat) %in% zero_samples, drop = FALSE]
    })
  }
...
initTransferLearningParamaters <- function(
  ...
  Fctrzn_Lrn_W0 <- sapply(
    views,
    function(view, Fctrzn, YTrgFtrs) {
      ...
      Fctrzn_Lrn_W0 = Fctrzn_Lrn_W0[match(
        YTrgFtrs[[view]],
        names(Fctrzn_Lrn_W0)
      )]
  ...
  Fctrzn_Lrn_W <- sapply(
    ...
    function(view, Fctrzn, YTrgFtrs) {
      ...
      Fctrzn_Lrn_W = Fctrzn_Lrn_W[
        match(YTrgFtrs[[view]], rownames(Fctrzn_Lrn_W)),
      ]
      return(Fctrzn_Lrn_W)
    },
    Fctrzn,
  ...
  Fctrzn_Lrn_WSq <- sapply(
    ...
    function(view, Fctrzn, YTrgFtrs) {
      ...
      Fctrzn_Lrn_WSq = Fctrzn_Lrn_WSq[
        match(YTrgFtrs[[view]], rownames(Fctrzn_Lrn_WSq)),
      ]
  ...
  Tau <- sapply(
    views,
    function(view, Fctrzn, YTrg, YTrgFtrs) {
  ...
  TauLn <- sapply(
    views,
    function(view, likelihoods, Fctrzn, YTrg, YTrgFtrs) {
...
TauLn_calculation <- function(
  ...
  if (likelihoodsLrn[view] == "gaussian") {
    if (LrnSimple) {
      ...
      TauLn = as.vector(
        read.csv(
          file.path(LrnFctrnDir, paste0("TauLn_", view, ".csv")),
          header = FALSE
        )$V1
      )
...
WSq_calculation <- function(view, Fctrzn, LrnFctrnDir, LrnSimple = TRUE) {
  ...
  if (LrnSimple) {
    ...
    WSq = read.csv(
      file.path(LrnFctrnDir, paste0("WSq_", view, ".csv")),
      header = FALSE
    )
...
intercepts_calculation <- function(expdat_meta, Fctrzn, FctrznDir, ExpDataDir) {
  ...
  intercepts_list <- lapply(
    views,
    function(view, likelihoods, D, YTmp, expdat_meta, Fctrzn) {
      print(view)
      ...
      YTmp <- as.data.frame(data.table::fread(
        file = file.path(ExpDataDir, paste0(view, '.csv')),
        sep = ","
      ))
      YTmp = t(as.matrix(YTmp))
      rownames(YTmp) = expdat_meta$smpls
      colnames(YTmp) = expdat_meta[[which(
        names(expdat_meta) == paste0("ftrs_", view)
      )]]
      ...
      if (likelihood == "gaussian") {
        ...
        Intercepts_df <- do.call(
          rbind,
          lapply(
            c(1:DTmp),
            function(d, YTmp, ZWTmp) {
              ...
              nLL = function(interceptMLE) {
                -sum(log(
                  stats::dpois(YTmp_d, log(1 + exp(ZWTmp_d + interceptMLE)))[
                    stats::dpois(
                      YTmp_d[, d],
                      log(1 + exp(ZWTmp_d + interceptMLE))
                    ) !=
                      0
                  ]
                ))
              ...
              interceptMLEfit = try(as.vector(stats4::mle(
                nLL,
                start = list(interceptMLE = InterceptsNaive[d])
              )@coef[1]))
        ...
        InterceptsMethod = setNames(
          Intercepts_df$Method,
          row.names(Intercepts_df)
        )
        ...
        Intercepts_df <- do.call(
          ...
          lapply(
            ...
            function(d, YTmp, ZWTmp) {
              ...
              nLL = function(InterceptMLE) {
                -sum(log(
                  dbinom(YTmp_d, size = 1, plogis(ZWTmp_d + InterceptMLE))[
                    dbinom(YTmp_d, size = 1, plogis(ZWTmp_d + InterceptMLE)) !=
                      0
                  ]
                ))
              ...
              interceptMLEfit = try(stats4::mle(
                nLL,
                start = list(InterceptMLE = InterceptsNaive[d])
              )@coef[1])
  ...
  EstimatedIntercepts <- list(
    "Seed" = Seed,
    "InterceptsNaive" = lapply(
      views,
      function(v, intercepts_list) {
...
YGauss_calculation <- function(view, likelihoods, YTrg, Zeta, Tau, CenterTrg) {
  ...
  if (likelihoods[[view]] == "poisson") {
    YGauss = Zeta[[view]] -
      plogis(Zeta[[view]]) *
        (1 - YTrg[[view]] / (log(1 + exp(Zeta[[view]])) + PoisRateCstnt)) /
        Tau[[view]]
...
ELBO_calculation <- function(
  ...
  if (likelihoods[[view]] == "poisson") {
    ...
    ELBO_L_tmpA = 0.5 *
      Tau[[view]] *
      (E_ZWSq[[view]] - 2 * E_ZE_W[[view]] * Zeta[[view]] + Zeta[[view]]^2)
    ELBO_L_tmpB = (E_ZE_W[[view]] - Zeta[[view]]) *
      plogis(Zeta[[view]]) *
      (1 - YTrg[[view]] / (log(1 + exp(Zeta[[view]])) + PoisRateCstnt))
    ELBO_L_tmpC = (log(1 + exp(Zeta[[view]])) + PoisRateCstnt) -
      YTrg[[view]] * log((log(1 + exp(Zeta[[view]])) + PoisRateCstnt))
    ...
    ELBO_L_tmpB = Tau[[view]] *
      (YGauss[[view]]^2 - 2 * YGauss[[view]] * E_ZE_W[[view]] + E_ZWSq[[view]])
...
E_ZE_W_update <- function(view, ZMu_0, ZMu, Fctrzn_Lrn_W0, Fctrzn_Lrn_W) {
  ...
  E_ZE_W = cbind(ZMu_0, ZMu) %*%
    t(cbind(Fctrzn_Lrn_W0[[view]], Fctrzn_Lrn_W[[view]]))
  return(E_ZE_W)
}
...
E_Z_SqE_W_Sq_update <- function(view, ZMu_0, ZMu, Fctrzn_Lrn_W0, Fctrzn_Lrn_W) {
  ...
  E_Z_SqE_W_Sq = (cbind(ZMu_0, ZMu)^2) %*%
    t(cbind(Fctrzn_Lrn_W0[[view]], Fctrzn_Lrn_W[[view]])^2)
  return(E_Z_SqE_W_Sq)
}

E_ZSqE_WSq_update <- function(
  view,
  ZMu_0,
  ZMuSq,
  Fctrzn_Lrn_W0,
  ...
  E_ZSqE_WSq = cbind(ZMu_0^2, ZMuSq) %*%
    t(cbind(Fctrzn_Lrn_W0[[view]]^2, Fctrzn_Lrn_WSq[[view]]))
  return(E_ZSqE_WSq)
}
...
VarExplFun <- function(views, YGauss, ZMu_0, Fctrzn_Lrn_W0, ZMu, Fctrzn_Lrn_W) {
  ...
  SS_tmp <- sapply(
    views,
    function(view, YGauss, ZMu_0, Fctrzn_Lrn_W0) {
      SS_tmp <- sum(
        (YGauss[[view]] -
          (matrix(ZMu_0, ncol = 1) %*% t(Fctrzn_Lrn_W0[[view]])))^2,
        na.rm = TRUE
      )
      return(SS_tmp)
    },
  ...
  VarExpl = sapply(
    views,
    function(view, YGauss, ZMu_0, ZMu, Fctrzn_Lrn_W0, Fctrzn_Lrn_W, SS_tmp) {
      ...
      var_expl_tmp = sapply(
        ...
        function(
          ...
          RSS_tmp = sum(
            (YGauss[[view]] -
              (cbind(ZMu_0, ZMu[, factorName]) %*%
                t(cbind(
                  Fctrzn_Lrn_W0[[view]],
                  Fctrzn_Lrn_W[[view]][, factorName]
                ))))^2,
            na.rm = TRUE
          )
...
transferLearning_function <- function(
  TL_param,
  MaxIterations,
  MinIterations,
  minFactors,
  ...
  for (It in 0:MaxIterations) {
    ...
    if (
      (BegK > minFactors) &
        (It > 1) &
        (It > StartDropFactor) &
        (((It - StartDropFactor - 1) %% FreqDropFactor) == 0)
    ) {
      print("Drop factors")
    ...
    if ((It > 0) & (It >= StartELBO) & (((It - StartELBO) %% FreqELBO) == 0)) {
      ...
      if (length(ELBO) >= 2) {
        ELBO_delta = 100 *
          abs((ELBO[length(ELBO)] - ELBO[length(ELBO) - 1]) / ELBO[1])
    ...
    if (
      (It >= 2) & (It >= MinIterations) & (convergence_token >= ConvergenceIts)
    ) {
      print("converged")
      break
    }
