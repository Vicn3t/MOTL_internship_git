...
downloadAllTCGAProjectData <- function(SaveDirectory) {
  ...
  lapply(
    X = ProjectsTCGA,
    FUN = function(Prjct, SaveDirectory) {
...
selectMinBarcodeSampleNames <- function(expdat, smpls_shared, Prjct) {
  ...
  brcds_expdat = data.frame(
    brcds = colnames(expdat)[is.element(smpls_names, smpls_shared)]
  ) %>%
...
mRNAFileParsing <- function(expdat_mRNA) {
  ...
  SmplsKeep_mRNA = (colMeans(is.na(expdat_mRNA_tmp)) < 0.2) &
    (colVars(expdat_mRNA_tmp, na.rm = TRUE) > 0)
...
miRNAFileParsing <- function(expdat_miRNA) {
  ...
  expdat_miRNA_tmp = expdat_miRNA[,
    substr(colnames(expdat_miRNA), 1, 10) == "read_count"
  ]
  colnames(expdat_miRNA_tmp) = substr(
    colnames(expdat_miRNA_tmp),
    12,
    nchar(colnames(expdat_miRNA_tmp))
  )
  expdat_miRNA_tmp = as.matrix(expdat_miRNA_tmp)
  mode(expdat_miRNA_tmp) = "integer"
  SmplsKeep_miRNA = (colMeans(is.na(expdat_miRNA_tmp)) < 0.2) &
    (colVars(expdat_miRNA_tmp, na.rm = TRUE) > 0)
...
DNAmeFileParsing <- function(expdat_DNAme) {
  ...
  SmplsKeep_DNAme = (colMeans(is.na(expdat_DNAme_tmp)) < 0.2) &
    (colVars(expdat_DNAme_tmp, na.rm = TRUE) > 0)
...
SNVFileParsing <- function(expdat_SNV) {
  ...
  expdat_SNV_parsed = expdat_SNV[
    is.element(
      expdat_SNV$Variant_Classification,
      c(
        "Frame_Shift_Del",
        "Frame_Shift_Ins",
        "In_Frame_Del",
  ...
  expdat_SNV_reshape = reshape2::acast(
    expdat_SNV_matrix,
...
mRNAPreprocessing <- function(Prjct, brcds_mRNA, InputDir) {
  ...
  SmplsKeep_mRNA = is.element(
    colnames(assay(expdat_mRNA_tmp)),
    brcds_mRNA$brcds
  )
  expdat_mRNA_tmp = expdat_mRNA_tmp[, SmplsKeep_mRNA]
...
miRNAPreprocessing <- function(Prjct, brcds_miRNA, InputDir) {
  ...
  expdat_miRNA_tmp = expdat_miRNA_tmp[,
    substr(colnames(expdat_miRNA_tmp), 1, 10) == "read_count"
  ]
  colnames(expdat_miRNA_tmp) = substr(
    colnames(expdat_miRNA_tmp),
    12,
  ...
  # samplewise filtering
  SmplsKeep_miRNA = is.element(
    colnames(assay(expdat_miRNA_tmp)),
    brcds_miRNA$brcds
  )
  expdat_miRNA_tmp = expdat_miRNA_tmp[, SmplsKeep_miRNA]
...
DNAmePreprocessing <- function(Prjct, brcds_DNAme, InputDir) {
  ...
  expdat_DNAme_tmp = SummarizedExperiment(
    assays = list(
      MValues = sesame::BetaValueToMValue(assay(expdat_DNAme_tmp, 1))
    ),
  ...
  # samplewise filtering
  SmplsKeep_DNAme = is.element(
    colnames(assay(expdat_DNAme_tmp)),
    brcds_DNAme$brcds
  )
  expdat_DNAme_tmp = expdat_DNAme_tmp[, SmplsKeep_DNAme]
...
SNVPreprocessing <- function(Prjct, brcds_SNV, InputDir) {
  ...
  expdat_SNV_tmp = expdat_SNV_tmp[
    is.element(
      expdat_SNV_tmp$Variant_Classification,
      c(
        "Frame_Shift_Del",
        "Frame_Shift_Ins",
        "In_Frame_Del",
  ...
  SmplsKeep_SNV = is.element(
    expdat_SNV_tmp$Tumor_Sample_Barcode,
    brcds_SNV$brcds
  )
...
mRNAdataPreparation <- function(Prjcts, brcds_mRNA, InputDir) {
  ...
  for (i in 1:length(Prjcts)) {
    ...
    if (i == 1) {
      ...
      exdat_mRNA = exdat_mRNA[
        match(rownames(expdat_mRNA_merged), rownames(exdat_mRNA)),
      ]
      expdat_mRNA_merged = SummarizedExperiment::cbind(
        expdat_mRNA_merged,
        exdat_mRNA
      )
    }
...
miRNAdataPreparation <- function(Prjcts, brcds_miRNA, InputDir) {
  ...
  for (i in 1:length(Prjcts)) {
    ...
    if (i == 1) {
      ...
      exdat_miRNA = exdat_miRNA[
        match(rownames(expdat_miRNA_merged), rownames(exdat_miRNA)),
      ]
      expdat_miRNA_merged = SummarizedExperiment::cbind(
        expdat_miRNA_merged,
        exdat_miRNA
      )
    }
...
DNAmedataPreparation <- function(Prjcts, brcds_DNAme, InputDir) {
  ...
  for (i in 1:length(Prjcts)) {
    ...
    if (i == 1) {
      ...
      exdat_DNAme = exdat_DNAme[
        match(rownames(exdat_DNAme_merged), rownames(exdat_DNAme)),
      ]
      exdat_DNAme_merged = SummarizedExperiment::cbind(
        exdat_DNAme_merged,
        exdat_DNAme
      )
    }
...
SNVdataPreparation <- function(Prjcts, brcds_SNV, InputDir) {
  ...
  exdat_SNV_merged = exdat_SNV_merged[,
    substr(colnames(exdat_SNV_merged), 1, 2) == "n."
  ]
  colnames(exdat_SNV_merged) = substr(
    colnames(exdat_SNV_merged),
    3,
    nchar(colnames(exdat_SNV_merged))
  )
...
DNAmeFiltering <- function(expdat_DNAme, TopD) {
  ...
  FtrsKeep_DNAme_fltr = base::rank(
    -rowVars(assay(expdat_DNAme_tmp), na.rm = TRUE),
    ties.method = "first"
  ) <=
    TopD
...
SNVFiltering <- function(expdat_SNV, TopD, SNVthreshold = 0.01) {
  ...
  FtrsKeep_SNV_fltr = base::rank(
    -rowVars(expdat_SNV_tmp, na.rm = TRUE),
    ties.method = "first"
  ) <=
    TopD
...
countsNormalization <- function(expdat, GeoMeans) {
  ...
  if (is.numeric(GeoMeans)) {
    ## normalization
    expdat_dds_norm = DESeq2::estimateSizeFactors(
      expdat_dds,
      geoMeans = GeoMeans
    )
    GeoMeans = NULL
  ...
  expdat_counts_norm <- list(
    "counts" = counts(expdat_dds_norm, normalized = TRUE)
  )
...
countsTransformation <- function(expdat_count, TopD) {
  ...
  FtrsKeep = base::rank(
    -rowVars(expdat_counts_log, na.rm = TRUE, useNames = FALSE),
    ties.method = "first"
  ) <=
    TopD
...
createSubsetOneProject <- function(
  SSnb,
  brcds_SS,
  if_SNV = FALSE,
  OutDir,
  ...
  expdat_mRNA_merged <- mRNAdataPreparation(
    Prjcts = Prjct,
    brcds_mRNA = brcds_mRNA_SS,
    InputDir = InputDir
  )

  ## Normalization
  expdat_mRNA_norm <- countsNormalization(
    expdat = expdat_mRNA_merged,
    GeoMeans = GeoMeans
  )

  ## Transformation
  expdat_mRNA_log <- countsTransformation(
    expdat_count = expdat_mRNA_norm$counts,
    TopD = TopD
  )

  ## Save data
  expdat_mRNA_sorted <- orderAndSaveData(
    expdat = expdat_mRNA_log,
    smpls = smpls_SS,
    OutDir = SSOutDir,
    fileName = "mRNA.csv"
  )
  ...
  ## Read and merge files
  expdat_miRNA_merged <- miRNAdataPreparation(
    Prjcts = Prjct,
    brcds_miRNA = brcds_miRNA_SS,
    InputDir = InputDir
  )

  ## Normalization
  expdat_miRNA_norm <- countsNormalization(
    expdat = expdat_miRNA_merged,
    GeoMeans = GeoMeans
  )

  ## Transformation
  expdat_miRNA_log <- countsTransformation(
    expdat_count = expdat_miRNA_norm$counts,
    TopD = TopD
  )

  ## Save data
  expdat_miRNA_sorted <- orderAndSaveData(
    expdat = expdat_miRNA_log,
    smpls = smpls_SS,
    OutDir = SSOutDir,
    fileName = "miRNA.csv"
  )
  ...
  ## Read and merge files
  expdat_DNAme_merged <- DNAmedataPreparation(
    Prjcts = Prjct,
    brcds_DNAme = brcds_DNAme_SS,
    InputDir = InputDir
  )

  ## Filtering
  expdat_DNAme_fltr <- DNAmeFiltering(
    expdat_DNAme = expdat_DNAme_merged,
    TopD = TopD
  )

  ## Save data
  expdat_DNAme_sorted <- orderAndSaveData(
    expdat = expdat_DNAme_fltr,
    smpls = smpls_SS,
    OutDir = SSOutDir,
    fileName = "DNAme.csv"
  )
  ...
  if (if_SNV) {
    ...
    expdat_SNV_merged <- SNVdataPreparation(
      Prjcts = Prjct,
      brcds_SNV = brcds_SNV_SS,
      InputDir = InputDir
    )
    ...
    ## Save data
    expdat_SNV_sorted <- orderAndSaveData(
      expdat = expdat_SNV_fltr,
      smpls = smpls_SS,
      OutDir = SSOutDir,
      fileName = "SNV.csv"
    )
  ...
  if (if_SNV) {
    expdat_list <- list(
      "mRNA" = expdat_mRNA_sorted,
      "miRNA" = expdat_miRNA_sorted,
      "DNAme" = expdat_DNAme_sorted,
      "SNV" = expdat_SNV_sorted
    )
    brcds_list <- list(
      "brcds_mRNA" = brcds_mRNA_SS,
      "brcds_miRNA" = brcds_miRNA_SS,
  ...
  saveMetadata(
    ...
    brcds_list = brcds_list,
    Prjcts = Prjcts,
    GeoMeans_list = NULL
  )
}

createSubsetMultiProjects <- function(
  SSnb,
  brcds_SS,
  if_SNV = FALSE,
  OutDir,
